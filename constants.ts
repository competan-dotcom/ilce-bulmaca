...
// Parse the RAW_DATA into an array of objects
const parsedData = RAW_DATA.trim().split('\n').map(line => {
  const [province, district] = line.split(',');
  return { province: province.trim(), district: district.trim() };
}).filter(item => !item.district.endsWith(' Merkez'));

// Shuffle the database using Fisher-Yates shuffle to ensure varied gameplay
for (let i = parsedData.length - 1; i > 0; i--) {
  const j = Math.floor(Math.random() * (i + 1));
  [parsedData[i], parsedData[j]] = [parsedData[j], parsedData[i]];
}

export const DISTRICT_DATABASE = parsedData;